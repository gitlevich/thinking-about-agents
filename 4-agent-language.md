...
The base unit in AL is contrast. A contrast is a dimension along which things can differ for an agent: light/dark,
near/far, safe/dangerous, “this code path” vs “that one.” A contrast isn’t a number. It’s a span. What matters is that
the agent can tell “more like this” vs “more like that.”

An observation is where a contrast lands right now. The field of possible differences collapses into “this much light,
this much distance, this particular path in the code.” Observations are always relative to contrasts. If there’s no
possible difference, there’s nothing to observe.

An observer is where observations register. An observer has no goal by default. It just lives in the stream of
...
room,” “make dinner,” “fix the bug,” “don’t think about my father’s death.” Once a goal is active, observations stop
...
goal that’s live, which possible next move is better, which is worse, which can be ignored. Preference doesn’t have to
be explicit. I can discover it by watching where attention actually goes.

Bandwidth in AL is a single abstract resource that action and meta-action both draw on. Generating narratives, planning,
tool calls, self-critique, even “just” staying present—they all cost. The details of energy, time, and compute are
hidden inside that one knob. When bandwidth is high, the agent can both act and monitor itself. When bandwidth is low,
action can crowd out monitoring. That was the story of capture.
...
serving the goal, but can’t afford to interrupt. Generation keeps consuming bandwidth. The regulator—the meta-process
that could cut the narrative and return to observer—also needs bandwidth, and gets starved. The spiral is just
preference running without supervision.

Goals in AL don’t have to be injected. In the previous piece I argued that they often precipitate from preference. In AL
terms: over time, preference fields carve grooves in the topology of contrasts. Attention falls into the same regions
again and again. The agent keeps making small choices that align along some direction. At some point that direction is
stable enough to be named as a goal. The goal is just a preference pattern that has crystallized into something the
agent can refer to.
...
You’d track bandwidth explicitly as something that both generation and meta-generation consume. The same abstract
resource that fuels tool calls and long chains of thought would also fuel the regulator that can cut them.

You’d represent goals not only as injected targets but also as crystallizations of preferences—patterns in where the

...
keeps firing long past usefulness. Standard response: add exit conditions, tune prompts, insert reflection steps. These
are patches. They don't touch the failure mode because there's no theory of what's happening.

Here's a model that comes from sustained observation of my own cognition and from thinking about how we might build
agents that share these failure modes.

I'm a late bilingual—Russian native, English acquired as an adult. This accident of biography left me with something
unusual: I can watch the conversion of experience into language. Most people's inner monologue is transparent to them.
Mine isn't. I have a tokenizer running, and I can see it tokenize.
...
The transition between observer and agent matters. A delta in the perceptual field—something salient, some
contrast—triggers a minimal agent: investigate the disturbance. Short trajectory, should complete and return to
...
Most agent architectures I see today lack an observer mode. They're always generating. There's no low-energy state to
...
As usually built, they have no explicit bandwidth model. Generation is treated as free. There's no representation of
meta-cognition requiring resources that generation might exhaust.

Their "reflection" is just more generation, with no capture detection. They can't recognize when they've fallen into a
gravity well.

And where there is self-monitoring, it rarely has teeth: it can't hard-interrupt and return to observer.

An alternative architecture would have explicit modes with different resource profiles. Observer is default. Agent is
entered deliberately and exited on trajectory completion or capture detection. Bandwidth is finite, depleted by
...
direction—not implemented here—would let goals precipitate from preference gradients through sustained attention. The
agent would develop goals the way minds do: by noticing what attention is already drawn toward, then rendering
continuity until the pattern crystallizes. That's a separate research program, on top of the architecture here.

The code below instantiates the mode/bandwidth/capture/regulation part of the model. Toy domain, but the control
structure is real. An agent that can observe, generate, get captured, detect capture, and recover. It traverses the
bookcase topology. The theory predicts the behavior.

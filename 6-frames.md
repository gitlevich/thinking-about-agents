...
I don’t build agent architectures. This is phenomenological description and architectural sketching, aimed at people who
do.

In Agent Language, a contrast is a dimension along which things can differ for an agent: light/dark, near/far,
safe/dangerous, “this design” vs “that one.” A contrast isn’t a number. It’s a span: “more like this end” or “more like
that end.” What matters is that the agent can tell the difference.

An observation is where a contrast lands right now. “This much light, this much distance, this much safety, this
specific design choice.” Observations are always relative to contrasts; without a possible difference there’s nothing to
observe.
...
Resolution in AL is how finely a contrast is sliced in this frame. I can register “it’s bright” or I can distinguish
between direct vs bounced light, cold vs warm, reflection vs emission. Same contrast, different resolution. Higher
resolution costs more bandwidth.
...
bit and I’ll treat it as “still the same moment.” That’s cheaper, but it means I miss some changes.

Bandwidth in AL is a single abstract resource that both resolution and frame rate draw on. If I track many contrasts at
high resolution and update them quickly, bandwidth goes fast. If I lower resolution, or slow the frame rate, or both, I
can stretch the same bandwidth further.
...
available bandwidth. If it doesn’t, something gives: I drop contrasts, I blur them, or I let the frame update more
slowly. That trade-off is not a choice the agent can avoid. It’s built in.
...
resolutions: beyond it, I’m just jittering between partial frames and not integrating anything. In AL terms, “attention
limit” is the boundary of the feasible region where number of contrasts, their resolution, and the frame rate all fit
under the bandwidth the agent actually has. Outside that region, degradation starts: missed signals, tunnel vision, or
collapse into a much simpler frame.
...
attention moves across contrasts. High bandwidth, low narrative cost. The sensorium flows through, but I’m not yet
turning it into story. It’s “what’s here,” not “what I’m doing.”
...
their side. Because I’m a late bilingual, I often see this projection happen: the frame is there first, then the words
arrive as a painting laid over it.
...
death. When I get captured by a thought, the writing frame can expand to fill attention, and the others go dim.
...
I’m willing to update them given my bandwidth and goal.”

An AL-flavored agent would make that explicit. At any moment it would internally track which contrasts it is currently
following at all, for each contrast what resolution is in play, and how fast it plans to update them, given bandwidth
and goals. Observer mode would be the regime where the agent mostly adjusts contrasts, resolution, and frame rate,
without committing to choices. Agent mode would be where it lets frames accumulate into choices and narratives.
